  ...
  <v-card flat tile class="no-top-bottom-height">
    ...
    <vue-loadmore
      ...
      <div class="notice-list">
        <div class="notice-item" v-for="(item, index) of list" @touchstart="showDialog(item.roomId)" @touchend="clearLoop(item.roomId)" :key="index">
          <span class="notice-title">{{item.roomName}}</span>
          <div class="meeting-info">
            <span class="meeting-time">地址: {{item.roomAddress}}</span>
          </div>
          <div class="meeting-info">
            <span class="meeting-time">人数: {{item.seatNumber}}</span>
          </div>
          <div class="meeting-info">
            <span class="meeting-time">备注: {{item.remark}}</span>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      id:'',
      items: [
        { text: '编辑会议室', icon: 'mdi-pencil' },
        { text: '删除会议室', icon: 'mdi-delete' },
  ...
  methods: {
    ...
    toAdd() {
      this.$router.push("/meetingManager/roomform");
    },
    toSearch() {
