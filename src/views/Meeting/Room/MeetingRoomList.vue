<template>
  <v-card flat tile>
    <v-data-table v-model="selected" :headers="th" :items-per-page="page.pageSize" :items="dataPage.content"
      ...
      @update:items-per-page="handleUpdatePageSize" show-select item-key="roomId" :loading="dataFlag"
      loading-text="加载中... 请稍后！">

      <template v-slot:top>
        <div id="Mtitle" class="d-flex flex-wrap pa-2 mb-2 tool-bar">
          <span>会议室名称：</span>
          <v-text-field full-width class="ma-2 flex-grow-0" label="会议室名称" hide-details dense outlined
            v-model="page.roomName" clearable></v-text-field>
          <span>会议室地址：</span>
          <v-text-field full-width class="ma-2 flex-grow-0" label="会议室地址" hide-details dense outlined
            v-model="page.roomAddress" clearable></v-text-field>
          <span>容纳人数：</span>
          <v-text-field full-width class="ma-2 flex-grow-0" label="容纳人数" hide-details dense outlined
            v-model="page.seatNumber" clearable></v-text-field>
          <v-btn class="ma-2" color="primary" @click="changeCurrentPage(1)">查询</v-btn>

          <v-btn class="ma-1" color="primary" @click="addOrEdit({})">新增1</v-btn>
...
export default {
  ...
  methods: {
    ...
    async getMeetingListData() {
      ...
      getMeettingList(this.page).then(res => {
